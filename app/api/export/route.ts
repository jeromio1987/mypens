...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (vaultModule === 'weight' || vaultModule === 'all') {
      ...
      if (vaultModule === 'all') parts.push('## WEIGHT\n' + toCSV(headers, rows))
      else parts.push(toCSV(headers, rows))
    }

    if (vaultModule === 'food' || vaultModule === 'all') {
      ...
      if (vaultModule === 'all') parts.push('## FOOD\n' + toCSV(headers, rows))
      else parts.push(toCSV(headers, rows))
    }

    if (vaultModule === 'sleep' || vaultModule === 'all') {
      ...
      if (vaultModule === 'all') parts.push('## SLEEP\n' + toCSV(headers, rows))
      else parts.push(toCSV(headers, rows))
    }

    if (vaultModule === 'training' || vaultModule === 'all') {
      ...
      if (vaultModule === 'all') parts.push('## TRAINING\n' + toCSV(headers, rows))
      else parts.push(toCSV(headers, rows))
    }

    if (vaultModule === 'measurements' || vaultModule === 'all') {
      ...
      if (vaultModule === 'all') parts.push('## MEASUREMENTS\n' + toCSV(headers, rows))
      else parts.push(toCSV(headers, rows))
    }

    const csv = parts.join('\n\n')
    const filename = vaultModule === 'all' ? 'my-pens-export.csv' : `my-pens-${vaultModule}.csv`
