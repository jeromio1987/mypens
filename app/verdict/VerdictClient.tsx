...
import type { VerdictData, VerdictPillar, LedgerItem } from '@/lib/verdictData'
...
export default function VerdictClient({
  ...
  const [data] = useState<VerdictData | null>(initialData)
  const [error] = useState<string | null>(initialError)
  const [aiSummary, setAiSummary]       = useState<string | null>(null)
  const [aiGeneratedAt, setAiGeneratedAt] = useState<string | null>(null)

  // AI summary only — main audit is SSR'd so HMR/StrictMode cannot leave skeletons forever.
  // No loading skeleton: optional garnish must not sit between score and pillars.
  useEffect(() => {
    let cancelled = false
    ...
    ;(async () => {
      try {
        const res = await fetch('/api/verdict/summary', { signal: ac.signal })
  ...
  return (
    <main className="min-h-screen bg-pens-deep">
      <div className="max-w-sm mx-auto px-5 pt-7 pb-28 space-y-7">
        ...
        {/* Weekly AI audit — only when we have text (no skeleton gap) */}
        {aiSummary && (
          <PremiumGate feature="AI weekly audit" className="rounded-2xl">
            <div className="rounded-2xl border border-pens-muted/30 bg-black/60 p-5 space-y-3">
