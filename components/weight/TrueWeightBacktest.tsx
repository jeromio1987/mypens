...
const SCENARIOS: Scenario[] = [
  {
    ...
    icon: '⚡',
    colorClass: 'border-pens-muted/30 bg-pens-deep/40',
...
const CONF_BADGE: Record<ConfidenceLevel, string> = {
  ...
  low:    'bg-pens-crimson/20 text-red-400 border border-pens-crimson/40',
}
    ...
    <div className={`rounded-xl border p-4 ${scenario.colorClass}`}>
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div>
            <p className="font-semibold text-pens-cream">{scenario.name}</p>
            <p className="text-xs text-pens-cream/50 mt-0.5">{scenario.description}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${pass ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' : 'bg-pens-crimson/20 text-red-400 border-pens-crimson/40'}`}>
            {pass ? '✓ Pass' : '⚠ Review'}
          </span>
          <span className="text-pens-cream/40 text-sm">{open ? '▲' : '▼'}</span>
        </div>
      </button>
      ...
      {open && (
        <div className="mt-4 space-y-4">
          {/* Expected behaviour */}
          <div className="text-xs text-pens-cream/70 bg-pens-deep/60 rounded-lg px-3 py-2 border border-pens-muted/30">
            <span className="font-medium text-pens-cream">Expected: </span>{scenario.expected}
          </div>

          {/* Mini chart */}
          <ResponsiveContainer width="100%" height={180}>
            <ComposedChart data={results} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#3D405B" strokeOpacity={0.3} />
              <XAxis dataKey="label" tick={{ fontSize: 10, fill: '#F5E6D3', fillOpacity: 0.6 }} stroke="#3D405B" />
              <YAxis domain={['auto', 'auto']} tick={{ fontSize: 10, fill: '#F5E6D3', fillOpacity: 0.6 }} unit=" kg" width={48} stroke="#3D405B" />
              <Tooltip
                contentStyle={{ background: '#0D1B2A', border: '1px solid #3D405B', borderRadius: '8px', color: '#F5E6D3' }}
                labelStyle={{ color: '#F5E6D3' }}
                cursor={{ fill: '#3D405B', opacity: 0.2 }}
              ...
              <Area type="monotone" dataKey="__bandWidth" stackId="b" stroke="none" fill="#C9A84C" fillOpacity={0.18} legendType="none" dot={false} activeDot={false} />
              <Line type="monotone" dataKey="baselineTrendKg" stroke="#7B6730" strokeWidth={1.5} strokeDasharray="5 3" dot={false} connectNulls={false} />
              <Line type="monotone" dataKey="trueWeightKg"    stroke="#C9A84C" strokeWidth={2} dot={{ r: 3, fill: '#C9A84C' }} />
              <Line type="monotone" dataKey="scaleKg"         stroke="#F5E6D3" strokeWidth={2} dot={{ r: 3, fill: '#F5E6D3' }} />
            </ComposedChart>
          </ResponsiveContainer>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="text-pens-cream/50">
              ...
              <tbody className="divide-y divide-pens-muted/20">
                {results.map((r, i) => (
                  <tr key={i} className={r.isSetup ? 'text-pens-cream/40' : 'text-pens-cream/80'}>
                    <td className="py-1.5 pr-3 font-medium">
                      {r.label}
                      {r.isOutlier && <span className="ml-1 text-red-400 text-xs">⚠</span>}
                    </td>
                    <td className={`py-1.5 pr-3 text-right tabular-nums ${r.isOutlier ? 'text-red-400 font-semibold' : ''}`}>
                      {r.scaleKg.toFixed(2)}
                    </td>
                    <td className="py-1.5 pr-3 text-right tabular-nums text-pens-gold">
                      {r.totalRetention > 0 ? `−${r.totalRetention.toFixed(2)}` : '—'}
                    </td>
                    <td className="py-1.5 pr-3 text-right tabular-nums text-emerald-400 font-semibold">
                      {r.trueWeightKg.toFixed(2)}
                    </td>
                    <td className="py-1.5 pr-3 text-right tabular-nums text-pens-cream/60">
                      {r.baselineTrendKg !== null ? r.baselineTrendKg.toFixed(2) : '—'}
                    </td>
                    <td className="py-1.5 pr-3 text-right tabular-nums text-pens-cream/40">
                      ±{r.bandKg}
                      {r.dynamicBandSource === 'dynamic' && <span className="text-pens-gold ml-0.5">*</span>}
                    </td>
                    <td className="py-1.5 text-right">
                      {r.isSetup ? (
                        <span className="text-pens-cream/30">setup</span>
                      ) : (
            ...
            <p className="text-xs text-pens-cream/40 mt-1.5">* = dynamic band (volatility-based). No asterisk = static fallback (insufficient history).</p>
          </div>

          {/* Pass condition note */}
          <p className="text-xs text-pens-cream/40">
            <span className="font-medium text-pens-cream/60">Pass condition:</span> {scenario.passCondition}
          </p>

          {!pass && (
            <div className="text-xs p-2 bg-amber-900/20 border border-amber-500/30 rounded-lg text-pens-gold">
  ...
  return (
    <div className="bg-pens-surface/80 border border-pens-muted/20 rounded-2xl p-6 w-full">
      <div className="flex items-start justify-between mb-1">
        <h2 className="text-xl font-semibold text-pens-cream">Backtest Harness</h2>
        <span className={`text-sm font-semibold px-2.5 py-1 rounded-full border ${passCount === SCENARIOS.length ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' : 'bg-amber-900/30 text-pens-gold border-amber-500/30'}`}>
          {passCount}/{SCENARIOS.length} pass
        </span>
      </div>
      <p className="text-xs text-pens-cream/40 mb-5">
