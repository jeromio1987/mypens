...
function generateInsights(data: {
  ...
  if (gapKg >= 0.5) {
    ...
    const reason = hasAlcohol
      ? 'alcohol-related water retention'
      : hasHighCarbs
        ? 'high-carb days loading glycogen'
        : 'water retention from logged confounders'
    insights.push({
      id: 'weight-gap',
      type: 'weight',
...
/** Shared by `/api/dashboard` and SSR `/dashboard` — parallel Prisma reads. */
export async function computeDashboardData(): Promise<DashboardData> {
  ...
  const [
    ...
    prisma.foodEntry.findMany({ where: { date: today } }),
    prisma.foodEntry.findMany({ where: { date: dateFilter } }),
    ...
    prisma.bodyMeasurement.findMany({ orderBy: { date: 'desc' }, take: 2 }),
    prisma.eventTag.findMany({ orderBy: { startDate: 'desc' } }),
  ])
  ...
  if (latestWeight) {
    ...
    latestTrueWeightKg = parseFloat(
      (
        latestWeight.scaleKg -
        latestWeight.creatineRetentionKg -
        latestWeight.alcoholRetentionKg -
        latestWeight.glycogenRetentionKg -
        sodiumKg -
        hardTrainingKg
      ).toFixed(2),
    )
  ...
  return {
    weight: {
      latest: latestWeight
        ? {
            ...
            trueWeightKg: latestTrueWeightKg as number,
    ...
    measurements: {
      latest: latestMeasurement
        ...
        : null,
      delta:
        latestMeasurement && prevMeasurement
          ? {
              waistCm:
                latestMeasurement.waistCm != null && prevMeasurement.waistCm != null
                  ? parseFloat((latestMeasurement.waistCm - prevMeasurement.waistCm).toFixed(1))
                  : null,
              chestCm:
                latestMeasurement.chestCm != null && prevMeasurement.chestCm != null
